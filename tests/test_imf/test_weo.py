...
from bblocks_data_importers.config import Fields, DataExtractionError, DataFormattingError
...
def mock_weo_data():
    ...
    data = {
        ...
        'CONCEPT_LABEL': ['Gross domestic product, deflator', 'Gross domestic product, deflator'],
...
def test_format_data(mock_weo_data):
    ...
    for old_col, new_col in expected_columns.items():
        ...
        assert old_col.lower() not in formatted_data.columns, f"Old column {old_col} still present"

    # Ensure all column names are lowercase
    assert all(col.islower() for col in formatted_data.columns), "Not all columns are lowercase"

    # Check that all dtypes are pyarrow types
    assert all(isinstance(dtype, pd.ArrowDtype) for dtype in formatted_data.dtypes), "Not all columns have pyarrow types"
...
def test_load_data_no_version_no_cached_data(mock_fetch_data, mock_weo_data):
    ...
    assert weo_importer._latest_version is None, "Latest version should be None initially"
    ...
    assert weo_importer._latest_version == ("April", 2023), "Latest version not updated correctly"
    assert ("April", 2023) in weo_importer._data, "Data not stored correctly for the version"

    # Ensure the fetched data is a DataFrame
    assert isinstance(weo_importer._data[("April", 2023)], pd.DataFrame), "Fetched data should be a DataFrame"


def test_load_data_with_specific_version(mock_fetch_data_specific_version, mock_weo_data):
    ...
    assert version in weo_importer._data, f"Data not stored correctly for version {version}"

    # Ensure the fetched data is a DataFrame
    assert isinstance(weo_importer._data[version], pd.DataFrame), "Fetched data should be a DataFrame"

    # Since we passed a specific version, _latest_version should not be updated
    assert weo_importer._latest_version is None, "Latest version should remain None when a specific version is passed"
...
def test_load_data_with_existing_data(mock_fetch_data, mock_weo_data):
    ...
    assert ("October", 2022) in weo_importer._data, "October 2022 data not preloaded correctly"
    ...
    assert ("October", 2022) in weo_importer._data, "October 2022 data missing after loading latest"
    assert ("April", 2023) in weo_importer._data, "Latest version data not cached correctly"

    # Ensure _latest_version is updated correctly to the latest version
    assert weo_importer._latest_version == ("April", 2023), "Latest version not updated correctly"

    # Ensure both cached datasets are DataFrames
    assert isinstance(weo_importer._data[("October", 2022)], pd.DataFrame), "October 2022 data should be a DataFrame"
    assert isinstance(weo_importer._data[("April", 2023)], pd.DataFrame), "Latest version data should be a DataFrame"
...
def test_load_data_data_extraction_error():
    ...
    with mock.patch('imf_reader.weo.fetch_data', side_effect=Exception("Fetch error")):
        ...
        with pytest.raises(DataExtractionError, match="Failed to fetch data: Fetch error"):
            weo_importer._load_data()
...
def test_load_data_data_formatting_error(mock_weo_data):
    ...
    with mock.patch('imf_reader.weo.fetch_data', return_value=mock_weo_data):
        ...
        with mock.patch.object(WEO, '_format_data', side_effect=Exception("Formatting error")):
            weo_importer = WEO()

            # Ensure _load_data raises DataFormattingError
            with pytest.raises(DataFormattingError, match="Error formatting data: Formatting error"):
                weo_importer._load_data()
...
def test_clear_cache(mock_fetch_data, mock_weo_data):
    ...
    assert weo_importer._latest_version == ("April", 2023), "Latest version not set correctly"
    ...
    assert weo_importer._latest_version is None, "Latest version should be None after clearing"
...
def test_get_data_with_cached_latest_data(mock_weo_data):
    ...
    assert weo_importer._latest_version == ("April", 2023), "Latest version not maintained correctly"
    assert ("April", 2023) in weo_importer._data, "Latest version data should still be cached"
...
def test_get_data_with_specific_version(mock_fetch_data_specific_version, mock_weo_data):
    ...
    # Ensure the specific version data is fetched and cached
    assert version in weo_importer._data, f"Data for version {version} not cached correctly"

    # Ensure the returned data is a DataFrame
    assert isinstance(data, pd.DataFrame), "Returned data should be a DataFrame"

    # Ensure the latest version is not updated when fetching a specific version
    assert weo_importer._latest_version is None, "Latest version should not be updated when fetching a specific version"
...
def test_get_data_with_cached_specific_version(mock_weo_data):
    ...
    assert version in weo_importer._data, f"Data for version {version} should still be cached"
