...
def test_format_data_maps_fields(baci, tmp_path, raw_df):
    ...
    (tmp_path / "product_codes_HS22_V202401.csv").write_text(
        "code,description\n010101,Foo product"
    )
...
def test_load_data_uses_existing_parquet(mock_val, baci, tmp_path):
    ...
    baci._load_parquet_dataset = MagicMock(
...
def test_get_data_loads_on_first_call(baci):
    ...
    baci._data = "mocked"
    assert (
        baci.get_data(country_format="iso3", product_description=False, years=[2022])
        == "mocked"
    )
